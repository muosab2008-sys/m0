<?php
	...
	$total_pages = $db->QueryGetNumRows("SELECT `id` FROM `completed_offers` WHERE `status`='3'");
	include(BASE_PATH.'/system/libs/paginator.php');

	$urlPattern = GenerateURL('canceled&x=(:num)', false, true);
...
<div class="pagetitle margin-top">
  ...
  <nav>
	<ol class="breadcrumb">
	  ...
	  <li class="breadcrumb-item active">Canceled Transactions</li>
	</ol>
  </nav>
</div>
	...
	<div class="col-lg-12">
	  <div class="table-responsive card">
		<table class="table table-striped table-hover table-sm table-responsive-sm text-center m-0">
			...
			<tbody>
				<?php
					$offers = $db->QueryFetchArrayAll("SELECT a.*, b.username, c.country FROM completed_offers a LEFT JOIN users b ON b.id = a.user_id LEFT JOIN list_countries c ON c.code = a.user_country WHERE a.status = '3' ORDER BY a.id DESC LIMIT ".$start.",".$limit);
					...
					foreach($offers as $offer)
					{
						echo '<tr>
							...
							<td><span class="badge bg-danger">$'.$offer['revenue'].'</span></td>
