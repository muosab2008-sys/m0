<?php
	...
	$total_pages = $db->QueryGetNumRows("SELECT `id` FROM `completed_offers` WHERE `status`='2' OR `status`='4'");
	include(BASE_PATH.'/system/libs/paginator.php');

	$urlPattern = GenerateURL('rejected&x=(:num)', false, true);
	$paginator = new Paginator($total_pages, $limit, $page, $urlPattern);
	$paginator->setMaxPagesToShow(5);
?> 
<main id="main" class="main">
<div class="pagetitle margin-top">
  <h1>Rejected Transactions (<?php echo number_format($total_pages); ?>)</h1>
  <nav>
	<ol class="breadcrumb">
	  <li class="breadcrumb-item"><a href="<?php echo GenerateURL('dashboard', false, true); ?>">Home</a></li>
	  <li class="breadcrumb-item active">Rejected Transactions</li>
	</ol>
  </nav>
</div>
	...
	<div class="col-lg-12">
	  <div class="table-responsive card">
		<table class="table table-striped table-hover table-sm table-responsive-sm text-center m-0">
			...
			<tbody>
				<?php
					$offers = $db->QueryFetchArrayAll("SELECT a.*, b.username, c.country FROM completed_offers a LEFT JOIN users b ON b.id = a.user_id LEFT JOIN list_countries c ON c.code = a.user_country WHERE a.status = '2' OR a.status = '4' ORDER BY a.id DESC LIMIT ".$start.",".$limit);
                  ...
                  x-show="1">'.$offer['reward'].'</span></td>
							<td>'.$offer['user_ip'].'</td>
							<td>'.($offer['status'] == 4 ? '<span class="badge bg-warning">Proxy / VPN</span>' : '<span class="badge bg-warning">Rejected</span>').'</td>
