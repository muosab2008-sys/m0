<?php
	...
	$total_pages = $db->QueryGetNumRows("SELECT `id` FROM `completed_offers` WHERE `status`='0'");
	include(BASE_PATH.'/system/libs/paginator.php');

	$urlPattern = GenerateURL('pending&x=(:num)', false, true);
	...
	if(isset($_GET['reject']) && is_numeric($_GET['reject']))
	{
		...
		$db->Query("UPDATE `completed_offers` SET `status`='2' WHERE `id`='".$id."'");
		$errMessage = '<div class="alert alert-warning"><b>SUCCESS:</b> Transaction was successfully rejected!</div>';
	}
?> 
	...
	<div class="col-lg-12">
	  ...
	  <div class="table-responsive card">
		<table class="table table-striped table-hover table-sm table-responsive-sm text-center m-0">
			...
			<tbody>
				<?php
					$offers = $db->QueryFetchArrayAll("SELECT a.*, b.username, c.country FROM completed_offers a LEFT JOIN users b ON b.id = a.user_id LEFT JOIN list_countries c ON c.code = a.user_country WHERE a.status = '0' ORDER BY a.id DESC LIMIT ".$start.",".$limit);

					if(empty($offers))
					{
						echo '<tr><td colspan="10" class="text-center">There is nothing here yet!</td></tr>';
					}
					...
					foreach($offers as $offer)
					{
						echo '<tr>
							...
							<td><span class="badge bg-light text-dark">'.$offer['reward'].' coins</span></td>
							<td>'.$offer['user_ip'].'</td>
							<td>'.date('d M Y H:i', $offer['timestamp']).'</td>
							<td><a href="'.GenerateURL('pending&reject='.$offer['id'], false, true).'" class="btn btn-sm btn-danger m-r-2" title="Reject" onclick="return confirm(\'Are you sure you want to reject this transaction?\');"><i class="bi bi-x-circle-fill"></i></a></td>
						  </tr>';
					}
				?>
			</tbody>
